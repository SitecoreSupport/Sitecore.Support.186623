...
Sitecore.FormBuilder = new function () {
    ...
    this.SaveData = function () {
        // Fix bug 186623
        this.SaveValidationMessageValue();
    ...
    this.SaveAndClose = function () {
        this.SaveValidationMessageValue();
        scForm.invoke('SaveFormStructureAndClose');
    }

    // Fix bug 186623
    this.SaveValidationMessageValue = function () {
        var textareaList = document.getElementsByTagName("textarea");
        if (textareaList != null) {
            for (var i = 0; i < textareaList.length; i++) {
                if (textareaList[i].id.indexOf("field_properties_loc") >= 0) {
                    var properties = Sitecore.Parser.parseXMLToArray(textareaList[i].value);
                    if (properties != null && properties.PredefinedValidatorTextMessage != null) {
                        textareaList[i].value = Sitecore.Parser.toXml(properties);
                    }
                }
            }
        }
